...
def train_epoch(dataloader, model, loss_fn, optimizer, use_ray=False):
    ...
    correct /= size
    print(f"this is from train_epoch Train Error: \n "
...
def validate_epoch(dataloader, model, loss_fn, Nclasses, use_ray=False):
    ...
    correct /= size
    print(
        f" this is from validate_epoch Test Error: \n "
        f"Accuracy: {(100 * correct):>0.1f}%, "
    ...
    for e in range(epochs):
        ...
        if use_ray:
            ...
            if best_loss > loss:
                ...
                model_name = f'model{config["wchannel"]}_{config["snr"]}_lgT.pt' if seq_len == 64 else f'model{config["wchannel"]}_{config["snr"]}_smT.pt'
                torch.save({
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wchannel", default='TGn', help="Wireless channel to be applied, it can be"
